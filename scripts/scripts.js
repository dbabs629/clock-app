...
let quoteAuthor = document.getElementsByClassName("quote-author");
...
let dayOfYear = Math.ceil(
  (date.getTime() - yearTimeStart) / (1000 * 60 * 60 * 24)
);
let currentWeek = Math.ceil(
  (date.getTime() - yearTimeStart) / (1000 * 60 * 60 * 168)
);
...
function checkTime(currentHour) {
  if (currentHour < 17 && currentHour >= 12) {
    ...
    backgroundImg[0].style.background =
      "rgba(0, 0, 0, 0.5) url('./assets/bg-image-daytime.jpg') no-repeat";
    ...
    backgroundImg[0].style.background =
      "rgba(0, 0, 0, 0.5) url('./assets/bg-image-nighttime.jpg') no-repeat";
    backgroundImg[0].style.backgroundSize = "cover";
    backgroundImg[0].style.animation = "fade-in";
    clock.style.animation = "fade-in";
    timeZoneAbbrev[0].style.animation = "fade-in";
...
//Retrieve data which contains quotes and authors from the rapidapi.com api
//too expensive to test the code and retrieve quotes each time it runs.
